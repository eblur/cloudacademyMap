...
def plot_opacity(chemical_species, cia_pairs, sigma_stored, cia_stored, P_desired, T_desired, wl_grid):
    ...
    ax.text(min(wl_grid)*1.20, 6.0e-23, (r'$\mathrm{T = }$' + str(T_desired) + r'$\mathrm{K \, \, P = }$' + str(P_desired*1000) + r'$\mathrm{mbar}$'), fontsize = 14)
...
T = np.linspace(800.0, 3000.0, 100)    # 100 temperatures linearly spaced from 800K -> 3000 K.
...
plot_opacity(chemical_species, cia_pairs, cross_sections[:,P_idx,T_idx,:], 
             cia_absorption[:,T_idx,:], P_desired, T_desired, wl)
